...
const AllHotels = () => {
    ...
    const adminGetHotels = async () => {
        try {
            const response = await axios.get("https://boovago-ljor.onrender.com/api/v1/hotels");
    ...
    const adminDeletehotel = async id => {
        try {
            const adminDeleteHotel = await axios.delete(`https://boovago-ljor.onrender.com/api/v1/hotels/${id}`);
            adminGetHotels();
        } catch (err) {
            console.error(err.message);
        }
